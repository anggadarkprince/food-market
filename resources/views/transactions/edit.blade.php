<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
        <x-jet-form-section actionUrl="{{ route('transactions.update', $transaction->id) }}" formMethod="post" enctype="multipart/form-data">
            <x-slot name="title">
                {{ __('Edit Transaction') }}
            </x-slot>

            <x-slot name="description">
                {{ __('Update transaction of food.') }}
            </x-slot>
            ...
            <x-slot name="form">
                ...
                <div class="col-span-6 md:col-span-3">
                    ...
                    <select name="food_id" id="food_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                        @foreach($foods as $food)
                            <option value="{{ $food->id }}"{{ old('food_id', $transaction->food_id) == $food->id ? ' selected' : '' }}>
                                {{ $food->food_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-span-6">
                    ...
                    <x-jet-input id="quantity" name="quantity" type="text" class="mt-1 block w-full" value="{{ old('quantity', $transaction->quantity) }}" autocomplete="price" placeholder="Order quantity" />
